...
from article_url import remove_repeat
...
class progarm(object):
    ...
    def post_data(self, news):
        ...
        if res.status_code == 200:
            ...
            print(self.time_now(), "\tOh My God,Send Failes @SKYNE url={}\tStatus_code ={}".format(news['link'], res.status_code))
        ...
        while(True):
            ...
            while (flag):
                if url_queue.is_empty():
                    ...
                    temp_url = url_queue.dequeue()
                    # 判断URL 是否被抓取过
                    if remove_repeat(temp_url):
                        news = self.get_news(url= temp_url)
                        if news:
                            time.sleep(3)
